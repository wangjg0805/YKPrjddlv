...
#include "iolpc2468.h"                        /* LPC23xx definitions */
...
void USB_Init (void) {
  ...
  /* The NXP Industrial Reference Design (IRD) uses U2 as USB device.          */
  /* This example will also work on the Embedded Artist LPC2468 board.         */
  /* To set OTG_STAT_CTRL register, OTG clock needs to be enabled              */
  OTG_CLK_CTRL = 0x1a;	                  /* Host, Dev, AHB clock enable       */
  while ((OTG_CLK_STAT & 0x1a) != 0x1a); 

 /* the default value is U1=device and U2=host, no change is required to the 
    OTG_STAT_CTRL register.  If we want to use U2 as the device port, then the
	OTG_STAT_CTRL = 3.	
	 This has to be set after OTG_CLK_CTRL configuration.  */
 //#if (USB_PORTSEL == 2) 
  OTG_STAT_CTRL = 3;					    /* USB Device Port Routing to U2   */
 //#endif
...
isr_end:
   //DEV_INT_CLR = disr;
   //DEV_INT_CLR = disr;
   WrCmd(CMD_GET_DEV_STAT);                  /* makes the suspend interrupt working */
