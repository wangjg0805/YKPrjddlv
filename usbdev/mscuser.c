...
#include "iolpc2468.h"                        /* LPC214x definitions */
...
BOOL MSC_Reset (void) {
 ...
 // FIO1SET = (LED_RD | LED_WR);  /* Turn Off R/W LED */
...
void MSC_MemoryRead (void) {
  ...
  if (BulkStage != MSC_BS_DATA_IN) {
    //FIO1SET = LED_RD;            /* Turn Off Read LED */
...
void MSC_MemoryWrite (void) {
  ...
  if ((Length == 0) || (BulkStage == MSC_BS_CSW)) {
    //FIO1SET = LED_WR;            /* Turn Off Write LED */
    CSW.bStatus = CSW_CMD_PASSED;
    MSC_SetCSW();
  }
}
...
fail: CSW.bStatus = CSW_CMD_FAILED;
      ...
      switch (CBW.CB[0]) {
        ...
        case SCSI_READ10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) != 0) {
              //FIO1CLR = LED_RD;    /* Turn On Read LED */
        ...
        case SCSI_WRITE10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) == 0) {
              //FIO1CLR = LED_WR;    /* Turn On Write LED */
